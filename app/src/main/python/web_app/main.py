...
import glob
import shutil
# Detect if we are on Android via environment variable
FFMPEG_PATH = os.environ.get("FFMPEG_PATH")
ANDROID_FILES_DIR = os.environ.get("ANDROID_FILES_DIR")

if FFMPEG_PATH and ANDROID_FILES_DIR:
    EXEC_DIR = ANDROID_FILES_DIR
    ffmpeg_link = FFMPEG_PATH
...
def cleanup_residues():
    try:
        ...
        if not os.path.exists(downloads_dir):
            return
        # Delete typical yt-dlp temporary files
        for root, _, files in os.walk(downloads_dir):
            for file in files:
                if file.endswith('.part') or file.endswith('.ytdl') or '.f' in file:
                    try:
...
async def serve_file(path: str):
    ...
    if not file_path.startswith(base_dir) or not os.path.exists(file_path):
        ...
        raise HTTPException(status_code=404, detail=f"Arquivo não encontrado: {path}. Arquivos na pasta: {files}")
